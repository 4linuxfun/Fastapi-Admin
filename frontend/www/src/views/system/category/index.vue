<template>
    ...
    <el-table :data="categoryData" style="width: 100%;margin-top: 10px;" border row-key="id">
        ...
        <el-table-column prop="code" label="分类编码" align="center"></el-table-column>
        <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope">
                <el-button link type="primary" @click="handleEdit(scope.row)">编辑</el-button>
                <el-button link type="primary" @click="handleDelete(scope.row)">删除</el-button>
                <el-button link type="primary" @click="handleAddChild(scope.row)">添加下一级</el-button>
            </template>
        </el-table-column>
    </el-table>
...
import { GetCategoryTree, LogicalDeleteCategory } from '@/api/category'
...
function handleAddChild(row) {
    ...
    addCategoryRef.value.addChild(row)
}

// 逻辑删除，子类也会同步标记为删除
async function handleDelete(category) {
    console.log('逻辑删除分类：', category)
    try {
        await ConfirmDel('确定逻辑删除分类:' + category.name + '?', LogicalDeleteCategory, category.id)
    } catch (e) {
