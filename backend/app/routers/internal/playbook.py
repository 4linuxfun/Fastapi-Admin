...
from app.models.internal.playbook import Playbook, PlaybookSearch
...
async def get_all_user(search: Pagination[PlaybookSearch],
    ...
    :return:
    """
    total = crud.internal.playbook.search_total(
        session, search.search, {'name': 'like'})
    logger.debug(total)
    playbooks: List[Playbook] = crud.internal.playbook.search(
        session, search, {'name': 'like'})
...
async def query_playbooks(query: Union[str, None] = None, session: Session = Depends(get_session)):
    playbooks: List[Playbook] = crud.internal.playbook.query_playbooks(
        session, query)
    playbook_list = [playbook.model_dump(exclude={'playbook'}) for playbook
                     in playbooks]
...
async def create_playbook(playbook: Playbook, session: Session = Depends(get_session)):
    ...
    crud.internal.playbook.insert(session, Playbook(
        **playbook.model_dump(exclude_unset=True)))
    return ApiResponse()
