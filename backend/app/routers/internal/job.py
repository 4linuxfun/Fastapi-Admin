...
from app.models.internal.job import JobAdd
from app.common.database import get_session, get_redis, get_rpyc
from app.common.response_code import ApiResponse, SearchResponse
from app.common.utils import get_task_logs
from app.models.internal import Pagination
from app.models.internal.job import JobSearch, JobLogs, JobLogSearch
...
async def modify_job(job: JobAdd, session: Session = Depends(get_session)):
    ...
    try:
        ...
        conn.root.modify_job(job.id,
                             kwargs={'job_id': job.id, 'targets': job.targets,
                                     'ansible_args': job.ansible_args},
...
async def job_logs(page_search: Pagination[JobLogSearch], session: Session = Depends(get_session)):
    ...
    total = crud.internal.job_logs.search_total(
        session, page_search.search, filter_type={'job_id': 'eq', 'type': 'eq'})
    jobs = crud.internal.job_logs.search(session, page_search, filter_type={
                                         'job_id': 'eq', 'type': 'eq'})
    logger.debug(jobs)
    return ApiResponse(
        data={
