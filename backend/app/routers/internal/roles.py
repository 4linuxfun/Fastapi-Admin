...
async def get_roles(search: Pagination[RoleBase], session: Session = Depends(get_session)):
    total = crud.internal.role.search_total(
        session, search.search, {'name': 'like', 'enable': 'eq'})
    logger.debug(total)
    roles: List[Role] = crud.internal.role.search(
        session, search, {'name': 'like', 'enable': 'eq'})
...
async def add_roles(role_info: RoleInsert, session: Session = Depends(get_session)):
    ...
    try:
        db_obj = crud.internal.role.insert(
            session, Role(**role_info.model_dump()))
...
@router.delete('/roles/{id}', summary='删除角色', dependencies=[Depends(Authority('role:del'))], response_model=ApiResponse)
async def del_role(id: int, session: Session = Depends(get_session)):
    try:
        ...
        if db_obj.name == 'admin':
            return ApiResponse(code=500, message='admin用户组无法删除')
        if len(db_obj.users) > 0:
            return ApiResponse(code=500, message='有用户关联此角色')
