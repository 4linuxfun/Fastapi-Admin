...
from app.models.internal.host import Host, Group, GroupWithChild, CreateHost, HostWithIp
from app.common.utils import Tree
...
async def get_host(id: int, session: Session = Depends(get_session)):
    ...
    return ApiResponse(
        data={**host.model_dump(exclude={'ansible_password',
                                'ansible_ssh_private_key'}), 'groups': host_groups}
    )
