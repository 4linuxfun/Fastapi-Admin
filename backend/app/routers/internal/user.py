...
async def get_all_user(search: Pagination[UserWithOutPasswd],
    ...
    :return:
    """
    total = crud.internal.user.search_total(
        session, search.search, {'name': 'like', 'enable': 'eq'})
    logger.debug(total)
    users = crud.internal.user.search(
...
async def update_password(user: UserUpdatePassword, session: Session = Depends(get_session)):
    crud.internal.user.update_passwd(
        session, uid=user.id, passwd=user.password)
    return ApiResponse()
...
@router.delete('/users/{uid}', summary='删除用户', dependencies=[Depends(Authority("user:del"))], response_model=ApiResponse)
async def delete_user(uid: int, session: Session = Depends(get_session)):
    try:
        ...
        return ApiResponse(code=500, message='此用户不存在')
    casbin_enforcer.delete_roles_for_user(f'uid_{user.id}')
    session.delete(user)
    session.commit()
    return ApiResponse()
