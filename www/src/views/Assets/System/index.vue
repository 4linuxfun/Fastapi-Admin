<template>
	<div style="background-color: papayawhip;padding: 10px;">
			...
			<el-form :model="searchForm">
				<el-row>
					<el-form-item label="资产类型">
						<category-select
							v-model:category="searchForm.category"
							placeholder="资产类型"
		...
		<el-row>
			...
			<template v-for="(filter,index) in searchForm.filters" :key="filter.field">
				<search-select
					:category_id="searchForm.id"
					v-model:filter="searchForm.filters[index]"
	...
	<multi-dialog
		...
		v-model:visible="multiDialog"
		:category="searchForm.id"
		@reload="handleSearch"
	></multi-dialog>
	<add-dialog
		v-if="addDialog.show"
		v-model:visible="addDialog.show"
		@reload="handleSearch"
...
export default {
	...
	setup() {
		...
		const category = computed(() => { return searchForm.category })
